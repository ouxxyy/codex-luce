...
- Kept the transition on compositor-friendly transform/opacity properties without adding layout reads to panel toggles, preserving the 1.3.1 performance fixes.

## 1.3.1 — 2026-07-18

- Fixed a renderer self-refresh loop by making DOM/class writes idempotent and ignoring skin-owned mutation records.
- Cached Luce task-dock geometry so metrics updates no longer force composer layout reads.
- Reworked quota metrics aggregation to incrementally summarize rollout files instead of retaining raw event arrays.
- Relaxed watcher polling and metrics cadence to reduce idle CPU on Apple Silicon.

## 1.3.0 — 2026-07-18

- Rebuilt Codex Luce with clean versioned home and task chassis assets; removed baked lights, controls and data.
- Added read-only quota-cycle runtime, token and remaining-usage aggregation from Codex SQLite/rollout data.
- Replaced activity/context gauges with buffered Nothing-style runtime/token readouts and a bounded mechanical quota needle.
- Corrected home cover geometry, task-dock alignment and native content/popover stacking.
- Restyled core menus and dialogs with OLED surfaces, monoline treatment and preserved native semantics.
...
- 内置多套可复现的抽象预设；安装后幂等播种到主题库，菜单栏或 `switch-theme-macos.sh` 可直接切换，且绝不覆盖 `custom-*` 用户主题。
...
- 开源树：示例横幅改为无角色抽象几何图；验证截图不入库；补充 NOTICE / README 商标与安全边界说明。
