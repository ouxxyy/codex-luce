...
/usr/bin/find "$TMP/codex-luce" -type f \( -name '.DS_Store' -o -name '._*' \) -delete
/bin/chmod 755 "$TMP/codex-luce"/*.command
/bin/chmod 755 "$TMP/codex-luce"/scripts/*.sh "$TMP/codex-luce"/tests/*.sh
/bin/rm -f "$ARCHIVE"
COPYFILE_DISABLE=1 /usr/bin/ditto -c -k --keepParent --norsrc --noextattr \
  "$TMP/codex-luce" "$ARCHIVE"
